...
# ╔═╡ 1d9b8118-30e4-454a-8cc6-a914cc61b5f1
begin
	using  ModelingToolkit, OrdinaryDiffEq, Distributions
	using  Plots, PlutoUI, LaTeXStrings, ColorSchemes
	# Set plot theme
	theme(:vibrant) 
...
# ╔═╡ da8e023f-d1e0-4099-8af1-baf8cf578169
md"""
# Initialization and useful Functions
"""

# ╔═╡ f4606b9d-598e-48f4-9bae-a6f84cd5c93c
...
# ╔═╡ 9f98b53b-09f9-40d5-a00b-4a2028014659
md"""
# Writing the Model
"""
...
begin
    ...
    F = A * B
    # Create a function object from the symbolic expression
end

# ╔═╡ c15dfca9-c13b-4d82-b544-05f2532544d1
md"""
Next we define all the parameters regarding the type distribution of workers and fimrs.
"""

# ╔═╡ ac8144f9-72de-4edb-985e-2dca4468c9b9
begin
    ...
    wuni = Uniform(x_bounds[1], x_bounds[2]) # They multiply 100 times the distribution of x don't know why
    funi = Uniform(y_bounds[1], y_bounds[2])
end

# ╔═╡ 1fe58b7c-0817-4f66-b0b9-a748c346523f
md"""
And parameters specifiying the type of assortativity of the model (something that can be easily infered from the primitives) and the number of points that will be used for numeric evaluation.
"""

# ╔═╡ 103aa624-f05b-46d8-baf5-1f00f6b2f063
begin
	nk = 6000 # Number of grid points
    # Specifiying model assortativity
    assortativity = "positive";
end

# ╔═╡ 8cb26b26-c71c-40fc-bb88-7407eab44672
md"""
Next we will use the closed form solution of the model to characterize the equilibrium as a system of symbolic ODE's that can be feed to an ODE solver.

**Note** that our ODE system includes only 2 equations dropping the one associate with $w'(x)$, this won't affect the solvability of the system and we can recover equilibrium wages from the first order condition:

$$w(x) = f_\theta(x,\mu(x),\theta(x))$$
"""

# ╔═╡ 9c31c6fd-0811-45f9-a6a3-0eeac613d8de
begin
    @variables μ(x), θ(x), w(x)

    H = 100 * (wuni.b - wuni.a)/(funi.b - funi.a)

    # Obtain the derivatives of the production function
	...
	# Next let's define the system of ODE's
	D = Differential(x)
	eqs = []
...
begin
	...
	# parameter values
	p  = [ω_A => 0.25, ω_B => 0.5, σ_A => 0.5]

	# Inteval of values for x
	xspan = (1.0,100.0)
...
$$\mu(x_0) = \mu_0 \qquad \text{and} \qquad \theta(x_0) = \theta_0$$

Possitive assortative matching give us one initial condition:

$$\mu(x_0) = \theta_0$$

But we are still unable to pindown $\theta_0$.

We can guess initial values for $\theta_0$ and plot the results:

Guess intial firm size: $(@bind guess_firm_size1 Slider(8:0.01:15))
Ploting $(@bind which_plot Select([
				  "1" => "μ(x)",
				  "2" => "θ(x)",
				  "3" => "w(x)",
				  "4" => "Π(x)",
				  "all" => all], default="all")) solution(s)"""
...
begin
	...
	# Solving the system
	sol = solve(prob,Tsit5(), saveat=range(x_bounds[1], stop=x_bounds[2], length=nk)
)
	# Create a matrix of solution
	matrix_sol = hcat(sol.t, sol[θ], sol[μ])
	# Calculate wages and put them in the matrix:
	wages = map(i -> eval_wages(matrix_sol[i, :]), 1:nk)
	matrix_sol = hcat(matrix_sol, wages)
	...
	if which_plot == "all"
		plot(plots..., layout = (2,2), size = (800,600), legend=:bottomright)
	else
		ind = parse(Int64, which_plot)
		plot(plots[ind], layout = (1,2), size = (800,600), legend=:bottomright)
	end
...
md"""
What help us to find the correct solution is that we know a terminal condition for $\mu(x)$:

$$\mu(x_1) = y_1$$

This turns the initial value problem in to a boundary condition problem which we can solve.

Unfortunately converting symbolic expressions in to numerically soplvable boundary problems in `DifferentialEquations.jl` is not implemented yet.

Fortunately implementing a shooting algorithm to solve this is very easy.

**Note** This algorithm probably can be made more efficient by implementing some stop conditions inside the integrator, but for this problem the proposed solution works very well.
"""

# ╔═╡ 1210ce21-aa56-4a48-9ecc-f3684cf0eb47
md"""
The next two functions implement a shooting algorithm to solve the boundary problem given a system of ODEs with boundary conditions. Note that this solution is problem specific but can be easily generalized.
"""
...
function shooting_method(sys, xspan, p; tol=1e-3, max_iter = 10, saveat=[])
	...
	u0 = [μ => 1.0, θ => guess_firm_size] 
	if length(saveat) == 0
	 	sol = solve_IVP(sys, u0, xspan, p)
	else
		sol = solve_IVP(sys, u0, xspan, p; saveat = 100)
	end
	n = 0
...
begin
	@time solution = shooting_method(sys, xspan, p; tol=1e-5, max_iter = 100)
	solution(100)
end
...
begin
	...
	plots_ = [
	plot(sol_, vars =(2), title="μ(x)", xlabel = "x", label="")
	plot(sol_, vars =(1), title="θ(x)", xlabel = "x", label="", ylims = (0,1000) )]

	plot(plots_..., layout = (1,2), size = (800,300), legend=:bottomright)
end
